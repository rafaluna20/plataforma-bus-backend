...
describe('PaymentService', () => {
    ...
    describe('processCardPayment()', () => {
        // Usa mockQueryRunner (no mockBookingRepo): al igual que processWalletPayment,
        // el cobro con tarjeta corre dentro de una transacción con bloqueo pesimista
        // sobre la reserva, para que dos cobros concurrentes sobre el mismo booking
        // no pasen ambos la validación "¿ya está pagada?" y cobren dos veces.
        it('debe procesar un pago con tarjeta exitosamente y marcar la reserva como PAID', async () => {
            ...
            mockQueryRunner.manager.save.mockImplementation((b) => Promise.resolve(b));
            ...
            expect(mockQueryRunner.manager.save).toHaveBeenCalledWith(booking);
            expect(mockQueryRunner.commitTransaction).toHaveBeenCalled();
            expect(mockQueryRunner.rollbackTransaction).not.toHaveBeenCalled();
        });

        it('debe usar bloqueo pesimista (pessimistic_write) al leer la reserva, para evitar doble cobro concurrente', async () => {
            const booking = makeBooking();
            mockQueryRunner.manager.findOne.mockResolvedValue(booking);
            mockQueryRunner.manager.save.mockImplementation((b) => Promise.resolve(b));

            jest.spyOn(paymentService as any, 'createCulqiCharge').mockResolvedValue({
                id: 'ch_test_lock', object: 'charge', amount: 5000, currency_code: 'PEN',
                email: cardPaymentData.email,
                outcome: { type: 'venta_exitosa', code: '000', merchant_message: 'ok', user_message: 'ok' },
                source: { id: 'tkn_test_123', type: 'card', card_number: '411111XXXXXX1111', brand: 'Visa' },
            });

            await paymentService.processCardPayment(cardPaymentData);

            expect(mockQueryRunner.manager.findOne).toHaveBeenCalledWith(expect.anything(), expect.objectContaining({
        ...
        it('debe lanzar error si el usuario no es dueño de la reserva', async () => {
            mockQueryRunner.manager.findOne.mockResolvedValue(makeBooking({ user: { id: 'otro-usuario' } }));
        ...
        it('debe lanzar error si la reserva ya fue pagada (PAID)', async () => {
            mockQueryRunner.manager.findOne.mockResolvedValue(makeBooking({ paymentStatus: PaymentStatus.PAID }));
        ...
        it('debe lanzar error si la reserva ya fue pagada (PAID_DIGITAL)', async () => {
            mockQueryRunner.manager.findOne.mockResolvedValue(makeBooking({ paymentStatus: PaymentStatus.PAID_DIGITAL }));
