...
class TripManagementService {
    get tripRepo() {
        return data_source_1.AppDataSource.getRepository(TripEntity_1.TripEntity);
    }
    /** Programar una nueva salida/viaje */
    async create(data) {
        const routeRepo = data_source_1.AppDataSource.getRepository(RouteEntity_1.RouteEntity);
        const vehicleRepo = data_source_1.AppDataSource.getRepository(VehicleEntity_1.VehicleEntity);
        ...
        const conflictingTrip = await this.tripRepo
            ...
            activeStatuses: [TripEntity_1.TripStatus.SCHEDULED, TripEntity_1.TripStatus.BOARDING, TripEntity_1.TripStatus.IN_TRANSIT],
        })
        ...
        logger_1.logger.info(`Viaje programado: ${saved.id} | Ruta: ${route.name} | Vehículo: ${vehicle.plateNumber} | Salida: ${departureDate.toISOString()}`);
        return saved;
    }
    ...
    async updateStatus(data) {
        ...
        const validTransitions = {
            [TripEntity_1.TripStatus.SCHEDULED]: [TripEntity_1.TripStatus.BOARDING, TripEntity_1.TripStatus.CANCELLED],
            [TripEntity_1.TripStatus.BOARDING]: [TripEntity_1.TripStatus.IN_TRANSIT, TripEntity_1.TripStatus.CANCELLED],
            [TripEntity_1.TripStatus.IN_TRANSIT]: [TripEntity_1.TripStatus.COMPLETED],
            [TripEntity_1.TripStatus.COMPLETED]: [],
            [TripEntity_1.TripStatus.CANCELLED]: [],
        };
        ...
        logger_1.logger.info(`Estado de viaje actualizado: ${data.tripId} | ${previousStatus} → ${data.status}`);
        return updated;
    }
    /** Listar viajes de una empresa con filtros opcionales y paginación */
    async findByCompany(companyId, status, options = {}) {
    ...
    async getPassengerManifest(tripId) {
...
//# sourceMappingURL=TripManagementService.js.map