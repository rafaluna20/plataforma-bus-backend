...
class BookingService {
    ...
    async validateAndCalculate(data, bookingRepo, tripRepo, waypointRepo, excludeStatuses) {
     ...
     */
    async createCashBooking(data) {
        const queryRunner = data_source_1.AppDataSource.createQueryRunner();
        await queryRunner.connect();
        await queryRunner.startTransaction('SERIALIZABLE');
        try {
            const tripRepo = queryRunner.manager.getRepository(TripEntity_1.TripEntity);
            const waypointRepo = queryRunner.manager.getRepository(RouteWaypointEntity_1.RouteWaypointEntity);
            const bookingRepo = queryRunner.manager.getRepository(BookingEntity_1.BookingEntity);
            const { trip, startWaypoint, endWaypoint, calculatedPrice } = await this.validateAndCalculate(data, bookingRepo, tripRepo, waypointRepo, [BookingEntity_1.PaymentStatus.CANCELLED, BookingEntity_1.PaymentStatus.REFUNDED, BookingEntity_1.PaymentStatus.FAILED]);
            ...
            logger_1.logger.info(`Reserva al contado creada: ${newBooking.id} | Asiento: ${data.seatId} | Precio: S/${calculatedPrice}`);
            return newBooking;
        }
        catch (error) {
     ...
     */
    async createDigitalBooking(data, paymentGateway, paymentDetails) {
        const queryRunner = data_source_1.AppDataSource.createQueryRunner();
        await queryRunner.connect();
        await queryRunner.startTransaction('SERIALIZABLE');
        try {
            ...
            const newBooking = bookingRepo.create({
                ...
                seatId: data.seatId,
                totalPrice: calculatedPrice,
                paymentStatus: BookingEntity_1.PaymentStatus.PENDING_DIGITAL,
            ...
            newBooking.paymentStatus = BookingEntity_1.PaymentStatus.PAID_DIGITAL;
            newBooking.paymentGatewayRef = paymentResult.transactionId ?? '';
            await bookingRepo.save(newBooking);
            await queryRunner.commitTransaction();
            logger_1.logger.info(`Reserva digital creada: ${newBooking.id} | Asiento: ${data.seatId} | Precio: S/${calculatedPrice} | Ref: ${newBooking.paymentGatewayRef}`);
            return newBooking;
        }
        catch (error) {
            await queryRunner.rollbackTransaction();
    ...
    async cancelBooking(bookingId, userId) {
        ...
        const cancellableStatuses = [BookingEntity_1.PaymentStatus.PENDING_CASH, BookingEntity_1.PaymentStatus.PENDING_DIGITAL];
        ...
        booking.paymentStatus = BookingEntity_1.PaymentStatus.CANCELLED;
        await bookingRepo.save(booking);
        logger_1.logger.info(`Reserva cancelada: ${bookingId}`);
        return booking;
    }
}
