...
const mockQueryRunner = {
    ...
    manager: {
        getRepository: jest.fn((entity) => {
            ...
            if (name === 'TripEntity')
                return mockTripRepo;
            if (name === 'RouteWaypointEntity')
                return mockWaypointRepo;
            return mockBookingRepo;
        }),
    },
};
...
describe('BookingService', () => {
    let bookingService;
    beforeEach(() => {
        bookingService = new BookingService_1.BookingService();
        jest.clearAllMocks();
    });
    describe('createCashBooking()', () => {
        it('debe crear una reserva al contado correctamente', async () => {
            ...
            expect(result.paymentStatus).toBe(BookingEntity_1.PaymentStatus.PENDING_CASH);
        ...
        it('debe calcular el precio sumando los tramos intermedios', async () => {
            ...
            mockWaypointRepo.find.mockResolvedValue(mockAllWaypoints);
            const capturedCreate = jest.fn().mockReturnValue({ id: 'b1', paymentStatus: BookingEntity_1.PaymentStatus.PENDING_CASH });
            mockBookingRepo.create = capturedCreate;
            mockBookingRepo.save.mockResolvedValue({ id: 'b1', paymentStatus: BookingEntity_1.PaymentStatus.PENDING_CASH });
            await bookingService.createCashBooking(baseBookingData);
            // El precio debe ser 25 (wp-junin) + 15 (wp-huancayo) = 40
            expect(capturedCreate).toHaveBeenCalledWith(expect.objectContaining({ totalPrice: 40 }));
        });
    ...
    describe('cancelBooking()', () => {
        it('debe cancelar una reserva PENDING_CASH', async () => {
            ...
            mockBookingRepo.save.mockResolvedValue({ ...booking, paymentStatus: BookingEntity_1.PaymentStatus.CANCELLED });
            const result = await bookingService.cancelBooking('b1');
            expect(result.paymentStatus).toBe(BookingEntity_1.PaymentStatus.CANCELLED);
        });
...
//# sourceMappingURL=BookingService.test.js.map