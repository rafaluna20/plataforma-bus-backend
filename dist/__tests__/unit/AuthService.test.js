...
describe('AuthService', () => {
    ...
    describe('register()', () => {
        ...
        it('debe normalizar el email a minúsculas', async () => {
            mockUserRepo.findOne.mockResolvedValue(null);
            mockUserRepo.create.mockReturnValue({ id: 'u1', email: 'juan@test.com', role: UserEntity_1.UserRole.PASSENGER, balance: 0, company: null });
            mockUserRepo.save.mockResolvedValue({ id: 'u1', email: 'juan@test.com', role: UserEntity_1.UserRole.PASSENGER, balance: 0, company: null });
            mockUserRepo.update.mockResolvedValue({ affected: 1 });
    ...
    // ─── login ────────────────────────────────────────────────────────────────
    describe('login()', () => {
        const hashedPassword = bcryptjs_1.default.hashSync('SecurePass123', 10);
        const mockUser = {
            id: 'user-uuid-123',
            name: 'Juan Pérez',
            email: 'juan@test.com',
        ...
        it('debe lanzar error si la cuenta está desactivada', async () => {
            ...
            await expect(authService.login({ email: 'juan@test.com', password: 'SecurePass123' })).rejects.toThrow('Esta cuenta ha sido desactivada');
        });
        it('el access token debe contener el payload correcto', async () => {
            ...
            const decoded = jsonwebtoken_1.default.verify(result.accessToken, process.env.JWT_SECRET);
